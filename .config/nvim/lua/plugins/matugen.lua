...
local function source_matugen()
  ...
  local matugen_path = os.getenv("HOME") .. "/.config/neovim/lua/colors.lua"  -- dofile doesn't expand $HOME or ~
...
local function auxiliary_function()
  ...
  local matugen_path = os.getenv("HOME") .. "/.dotfiles/home-manager/modules/neovim/nvim/plugin/template.lua"
  source_matugen()

  -- Because reloading base16 overwrites lualine configuration, just source lualine here
  dofile(os.getenv("HOME") .. '/.config/neovim/lua/colors.lua') -- path of your lualine setup
